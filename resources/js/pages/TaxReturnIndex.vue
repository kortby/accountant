...
const handleSearch = debounce((value) => {
    router.get(
        '/tax-returns',
        { search: value, status: statusFilter.value === 'all' ? null : statusFilter.value },
...
const handleStatusChange = (value) => {
    ...
    router.get(
        '/tax-returns',
        { search: search.value, status: value === 'all' ? null : value },
        { preserveState: true, replace: true }
    );
};
...
const getStatusColor = (status) => {
    switch (status) {
        ...
        case 'needs_action': return 'bg-red-100 text-red-700 border-red-200 dark:bg-red-900/30 dark:text-red-400 dark:border-red-800';
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400">Manage and track all your tax return submissions.</p>
                        </div>
                        ...
                        <div class="pb-4">
                            <Button><a href="/file-taxes-for-client">New File</a></Button>
                        </div>
                    </div>
                            ...
                            <TableBody>
                                ...
                                <TableRow v-for="tax in taxReturns.data" :key="tax.id">
                                    <TableCell class="font-medium">
                                        ...
                                        <div class="text-xs text-gray-500 dark:text-gray-400 font-normal">#{{ tax.id }}</div>
                                    </TableCell>
                                    <TableCell>
                                        <span
                                    ...
                                    <TableCell>
                                        <div v-if="tax.status === 'completed'">
                                            <span v-if="tax.refund_amount > 0" class="text-green-600 dark:text-green-400 font-medium">
                                                +{{ formatCurrency(tax.refund_amount) }}
                                            </span>
                                            <span v-else-if="tax.amount_due > 0" class="text-red-600 dark:text-red-400 font-medium">
                                                -{{ formatCurrency(tax.amount_due) }}
                                            </span>
                                            <span v-else class="text-gray-500 dark:text-gray-400">-</span>
                                        </div>
                                        <div v-else class="text-gray-400 dark:text-gray-500 italic text-xs">Processing</div>
                                    </TableCell>
                                    <TableCell class="text-right">
                                        <div class="flex justify-end gap-3">
                                            <Link :href="'/tax-returns/' + tax.id" class="text-orange-600 dark:text-orange-400 hover:text-orange-700 dark:hover:text-orange-300 font-medium text-sm">
                                                View
                                            </Link>
                                            <Link v-if="isAccountant" :href="'/tax-returns/' + tax.id + '/edit'" class="text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-medium text-sm">
                                                Edit
                                            </Link>
                                        </div>
                                    </TableCell>
